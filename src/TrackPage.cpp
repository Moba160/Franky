...
void TrackPage::buttons() {

  M5Btn::clearFunctions();
...
void TrackPage::buttonPressed(M5Btn::ButtonType button) {
  ...
  } else if (M5Btn::getFunction(button) == FN_DURCHF) {
    routeFilter = RT_THROUGH; matchFirstRoute();

  } else if (M5Btn::getFunction(button) == FN_EINF_HL) {
    routeFilter = RT_ELEFT; matchFirstRoute();

  } else if (M5Btn::getFunction(button) == FN_EINF_AHB) {
    routeFilter = RT_ERIGHT; matchFirstRoute();

  } else if (M5Btn::getFunction(button) == FN_AUSF_HL) {
    routeFilter = RT_ALEFT; matchFirstRoute();

  } else if (M5Btn::getFunction(button) == FN_AUSF_AHB) {
    routeFilter = RT_ARIGHT; matchFirstRoute();

  } else if (M5Btn::getFunction(button) == FN_FILT1) {
    routeGroup = 1;
    buttons();

  } else if (M5Btn::getFunction(button) == FN_FILT2) {
    routeGroup = 2;
    buttons();

  } else if (M5Btn::getFunction(button) == FN_SH_TOP) {
    routeFilter = RT_SH_TOP; matchFirstRoute();

  } else if (M5Btn::getFunction(button) == FN_SH_Q) {
    routeFilter = RT_SH_Q; matchFirstRoute();

  } else if (M5Btn::getFunction(button) == FN_SH_S) {
    routeFilter = RT_SH_S; matchFirstRoute();

  } else if (M5Btn::getFunction(button) == FN_SH_BOT) {
